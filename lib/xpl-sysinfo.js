...
var proto = {
	...
	/*
		...
		var json = {
			"device" : os.hostname(),
			"type": "freemem",
			"current": Math.round(osu.freemem()),
			"unit": "Mb"
		};
		self._sendXplTrigOrStat('freemem', json.type, json);
        },
		...
		var json = {
			"device" : os.hostname(),
			"type": "totalmem",
			"current": Math.round(osu.totalmem()),
			"unit": "Mb"
			
		};
		self._sendXplTrigOrStat('totalmem', json.type, json);
        },
		...
		var json = {
			...
			"current": Math.round(osu.freememPercentage()*100),
			"unit": "%"
		};
		self._sendXplTrigOrStat('freememPercentage', json.type, json);
        },
        ...
        send_harddrive: function(){  
			...
			self._sendXplTrigOrStat('diskTotal', json1.type, json1);
			...
			self._sendXplTrigOrStat('diskFree', json2.type, json2);
			...
			self._sendXplTrigOrStat('diskUsed', json3.type, json3);
                });
        },*/
        ...
        /*send_cpuFree: function(){  
			...
			self._sendXplTrigOrStat('cpuFree', json.type, json);
                });
        },*/
	...
	send_memUsage: function(){  
                var self = this;		
		var json = {
			"device" : os.hostname(),
			"type": "memUsage",
			"current": 100 - Math.round(osu.freememPercentage()*100),
			"unit": "%"
		};
		self._sendXplTrigOrStat('memUsage', json.type, json);
        },
        ...
        send_cpuUsage: function(){  
			...
			self._sendXplTrigOrStat('cpuUsage', json.type, json);
                });
        },
	
        send_diskUsage: function(){  
                var self = this;
                osu.harddrive(function(total, free, used){
			var json = {
				"device" : os.hostname(),
				"type": "diskUsage",
				"current": Math.round(used * 100 / total),
				"unit": "%"
			};
			self._sendXplTrigOrStat('diskUsage', json.type, json);
                });
        },
}
